## [CSS collection](https://alexeyvolkov.github.io/)

Project collects best accessability practices and cutting-edge CSS HTML technologies

### Fonts

- [Ubuntu Regular](https://design.ubuntu.com/font/)
- [JetBrains Mono](https://www.jetbrains.com/lp/mono/)
- [Mozilla's Zilla Slab Type Family](https://github.com/mozilla/zilla-slab)

### Dev Notes

- [LESS to CSS](https://jsonformatter.org/less-to-css)
- [Encode HTML special chars](https://codebeautify.org/html-encode-string)
- [CanIUse](https://caniuse.com/#feat=css-hyphens)
